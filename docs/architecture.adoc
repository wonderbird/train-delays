:gitplant: http://www.plantuml.com/plantuml/proxy?src=https://raw.githubusercontent.com/wonderbird/train-delays/feature/web-app/docs/plantuml
...
For main branch builds, the link:../.github/workflows/gradle.yml[build pipeline] creates a docker image containing the build artifact. This image is pushed to https://hub.docker.com[Docker Hub].

Users run the image on their Docker instance.

Further details are described in the link:user-guide.md[User Guide].
